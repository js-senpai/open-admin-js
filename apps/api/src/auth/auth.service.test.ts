...
describe("AuthService", () => {
  it("login succeeds for valid credentials and admin realm", async () => {
    ...
    const result = await service.login("admin@example.com", "password", {}, "admin");
  ...
  it("login fails for wrong realm", async () => {
    ...
    const service = new AuthService(prisma as never, jwt as never);
    await expect(service.login("admin@example.com", "password", {}, "admin")).rejects.toBeInstanceOf(UnauthorizedException);
  });
