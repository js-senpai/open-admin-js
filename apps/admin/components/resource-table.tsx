...
import { ArrowDown, ArrowUp, ArrowUpDown, ChevronLeft, ChevronRight, Plus, Search, SlidersHorizontal } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2 animate-in stagger-2">
        <div className="flex flex-1 items-center gap-2.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-all focus-within:border-[#2454ff] focus-within:shadow-[0_0_0_3px_rgba(36,84,255,0.1)] dark:border-slate-700 dark:bg-slate-900">
        ...
        <button aria-label="Filters" className="btn-ghost flex items-center gap-2 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.04),0_8px_24px_rgba(0,0,0,0.04)] animate-in stagger-3 dark:border-slate-700 dark:bg-slate-900">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="border-b border-slate-100 bg-slate-50/70 dark:border-slate-700 dark:bg-slate-800/50">
              <tr>
                {fields.map(([name, field]) => {
                  ...
                  return (
                    <th key={name} className={`px-4 py-3 ${sortable ? "cursor-pointer select-none" : ""}`}
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                Array.from({ length: 6 }).map((_, i) => <SkeletonRow key={i} cols={fields.length} />)
              ) : rows.length > 0 ? (
                rows.map((row, index) => (
                  <tr key={String(row.id ?? index)} className="group transition-colors hover:bg-blue-50/30 dark:hover:bg-slate-800/50">
                    {fields.map(([name, field]) => (
                      <td key={name} className="px-4 py-3.5 text-slate-700 dark:text-slate-300">
                    ...
                    <td className="px-4 py-3.5 text-right">
                      <Link
                        href={`/resources/${resource.name}/${String(row.id ?? "new")}`}
                ...
                <tr>
                  <td className="py-16 text-center" colSpan={fields.length + 1}>
        ...
        {/* Simple row count when no pagination */}
